...
use super::{Constraints, Layout, LayoutCx, RawScope, sizing_range};
...
impl Grid {
    ...
    /// enables automatic row and column spans with equal-height rows
...
/// resolved placement of a spanning grid child
...
/// child declaration scope for [`Grid`]
pub struct GridScope<'ui> {
    raw: RawScope<'ui, Grid>,
    next_row: [u16; MAX_SPANNING_COLUMNS],
    row: u16,
    column: u16,
}

impl<'ui> From<RawScope<'ui, Grid>> for GridScope<'ui> {
    fn from(raw: RawScope<'ui, Grid>) -> Self {
        Self {
            raw,
...
impl GridScope<'_> {
    ...
    pub fn add<W: Widget>(&mut self, widget: W) -> W::Output {
        if self.raw.layout.spanning {
            self.add_span(1, 1, widget)
        } else {
            widget.render(self.raw.ui)
        }
    }

    /// renders a child spanning the requested number of rows and columns
    pub fn add_span<W: Widget>(&mut self, rows: usize, columns: usize, widget: W) -> W::Output {
        assert!(
            self.raw.layout.spanning,
        ...
        assert!(
            column_span <= self.raw.layout.columns,
            "grid column span exceeds its column count"
        );

        let grid_columns = self.raw.layout.columns as usize;
        ...
        for column in 0..=grid_columns - span {
            ...
            if placement
                .is_none_or(|(best_row, best_column)| (row, column) < (best_row, best_column))
            {
    ...
    fn layout(&self, cx: &mut LayoutCx<'_, Self::Item>, constraints: Constraints) -> LogicalSize {
        ...
        if self.spanning {
            ...
            for node in cx.children() {
                ...
                let width = sizing_range(cx.sizing(node, super::Axis::Horizontal), f32::INFINITY);
                let height = sizing_range(cx.sizing(node, super::Axis::Vertical), max_height);
