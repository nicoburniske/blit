...
fn draw_commands<const CLIPPED: bool, P: Pixel>(
    ...
    for command in active {
                ...
                if start < clipped.full_start || end > clipped.full_end {
                    ...
                    .for_each(
                        ...
                        |range, coverage| {
                            ...
                            if let Payload::Polyline(command, segments) = payload {
                                raster::draw_polyline(
                                    command, segments, line, clip, coverage, polyline, buffer,
                                );
                            } else {
                                raster::draw_line(
                                    &payload, line, clip, coverage, images, text, buffer,
                                );
                            }
                        },
                    );
        ...
        let payload = commands.get(*command);
        if let Payload::Polyline(command, segments) = payload {
            raster::draw_polyline(command, segments, line, clip, 255, polyline, buffer);
        } else {
            raster::draw_line(&payload, line, clip, 255, images, text, buffer);
        }
    }
}
