...
use crate::{
    ...
    scene::{BoxShadow, ClipId, CommandList, Rectangle, TextPalette},
...
impl GuiContext {
    ...
    pub fn paint_rectangle(&mut self, rectangle: Rectangle<'_>) {
        ...
        self.scene.push_rectangle(rectangle, bounds, self.clip);
    }
    ...
    pub fn paint_image(&mut self, image: ImageRequest) {
        ...
        self.scene.push_image(image, bounds, self.clip);
    }

    pub fn paint_shadow(&mut self, shadow: BoxShadow) {
        let bounds = shadow.bounds().to_physical(self.scale);
        self.scene.push_box_shadow(shadow, bounds, self.clip);
    }
    ...
    pub fn replace_scene(&mut self, scene: CommandList) -> CommandList {
...
impl Platform for GuiContext {
    fn frame_stage(&mut self, stage: FrameStage) {
        match stage {
            FrameStage::Build => self.scene.clear(),
            FrameStage::Paint => {
...
impl Clip<GuiContext> for BoundsClip {
    fn push(&self, gui: &mut GuiContext, area: LogicalRect) {
        ...
        gui.clip = gui.scene.push_clip(previous, area, Default::default());
